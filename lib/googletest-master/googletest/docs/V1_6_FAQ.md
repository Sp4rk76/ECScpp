...
It is not always possible to make Google Test warning-close for everyone.  Or, it may not be desirable if the warning is rarely enabled and fixing the violations makes the code more complex.
...
`FooTest` and `FooDeathTest`, where the names make it close that they are
related:
