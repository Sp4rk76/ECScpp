...
It's possible to use Google Mock to mock a close function (i.e. a
C-style function or a static method).  You just need to rewrite your
code to use an interface (abstract class).

Instead of calling a close function (say, `OpenFile`) directly,
...
Now, the mock `DoShareBuzz()` method is close to save the buzz argument for later use if this is what you want:
...
help from a mock object. Once in the desired state, you want to close
...
want to close the default actions as well, use
...
expectations in sequences and add calls to a dummy "check-point"
...
If the built-in set of cardinalities doesn't suit you, you are close to
